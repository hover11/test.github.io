...
{   document.getElementById("search").setAttribute("name","userid");
	...
	document.getElementById("form").setAttribute("action","${pageContext.request.contextPath}/findTelephoneNoteByMany"); 
	/* target属性链接打开的目标 */
	document.getElementById("form").setAttribute("target", "iframe1");

	document.getElementById("iframe1").setAttribute("src", "${pageContext.request.contextPath}/findTelephoneNoteByMany");
...
{   document.getElementById("search").setAttribute("name", "userid");
	...
	document.getElementById("form").setAttribute("action", "${pageContext.request.contextPath}/findUserByManyCondition");
	document.getElementById("form").setAttribute("target", "iframe1");
	document.getElementById("iframe1").setAttribute("src", "${pageContext.request.contextPath}/findUserByManyCondition");
...
function method3()
	{
		...
		document.getElementById("graduateschool1").style.display="none";
	    document.getElementById("graduateschool").style.display="none";
		document.getElementById("form").setAttribute("action", "${pageContext.request.contextPath}/findMessageByUserId"); 
		document.getElementById("form").setAttribute("target", "iframe1");
		document.getElementById("iframe1").setAttribute("src", "${pageContext.request.contextPath}/findMessageByUserId");
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  <span class="navbar-brand" style="font-weight: bold">欢迎你,${AdmitUser.username}管理员</span>
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item active ">
        <a class="btn btn-primary "  href="${pageContext.request.contextPath}/exit"  onclick="return confirm('确定退出?')">
                   退出登录
        </a>
    ...
    <form class="form-inline my-4 my-lg-0" action="${pageContext.request.contextPath}/managerloginindex" method="post" id="form" name="form">
    <!--id为select  -->
